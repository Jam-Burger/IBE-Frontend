...
import {CheckoutSection} from './types';
...
const TravelerInfoSection: React.FC<TravelerInfoSectionProps> = ({
    ...
    const handleCustomInputChange = (
        ...
        // Get the correct field key for Redux store
        const fieldKey = fieldMapping[field.label] || field.name;
    ...
    const handleCustomNextStep = () => {
        ...
        section.fields.forEach(field => {
            if (field.enabled && field.required) {
                const fieldKey = fieldMapping[field.label] || field.name;
                const isValid = !!formData[fieldKey];
                newValidation[field.name] = isValid;
        ...
        const hasValidationErrors = Object.keys(formErrors)
            .filter(key => key.startsWith('traveler_'))
            .length > 0;
