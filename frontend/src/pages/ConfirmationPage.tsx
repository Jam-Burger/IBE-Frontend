...
import ErrorDialog from "../components/ui/ErrorDialog.tsx";
...
const ConfirmationPage: FC = () => {
        ...
        const handleCancelBooking = async (otp: string | null = null) => {
            if (!tenantId || !bookingId) {
        ...
        const handleCancelClick = async () => {
            if (auth.user?.profile.email) {
                const response = prompt("Do you want to cancel the booking? Type 'YES' to confirm.");
                if (response === "YES") {
                    handleCancelBooking();
                }
                return;
            ...
            if (!tenantId || !bookingData?.guest_details?.billingEmail) {
            ...
            try {
                await api.sendOtp(tenantId, bookingData.guest_details.billingEmail);
                setIsOTPModalOpen(true); // Open modal only on success
                toast.success(`OTP sent to ${bookingData.guest_details.billingEmail}`);
            } catch (error) {
        ...
        if (!bookingData || !propertyDetails || !roomTypeData) {
            if (loading) {
                ...
                return <div>
                    <ErrorDialog
                        title="Error"
                        message={error ?? "Failed to fetch booking details. Please try again later."}
                        onClose={() => {
                            navigate(`/${tenantId}`);
                        }}
                    />
        ...
        const handleEmailClick = async () => {
            ...
            try {
                ...
                toast.success(`Email sent to ${bookingData.guest_details.billingEmail}`);
            } catch (error) {
                ...
                <div
                    ...
                    <div className="px-2 py-6 relative">
                        ...
                        <OTPModal
                            ...
                            email={bookingData.guest_details.billingEmail}
